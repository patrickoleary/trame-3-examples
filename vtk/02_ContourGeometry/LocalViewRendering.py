...
# # VTK Head Contour with Local Rendering
#
...
# (isosurface) using server-side VTK geometry creation rendered locally via `VtkLocalView`.
#
# **Key Features:**
#
# - **VTK Pipeline**: Loads `head.vti`, applies `vtkContourFilter`.
# - **Local Rendering**: Uses `VtkLocalView` for displaying the VTK scene.
...
#   1. Make the script executable: `chmod +x ./LocalViewRendering.py` (if not already)
#   2. Run directly: `./LocalViewRendering.py`
#   Alternatively: `uv run ./LocalViewRendering.py`
#
...
#   `python ./LocalViewRendering.py --app`
#
# **Run in Jupyter Lab / Notebook:**
#   Rename this script (e.g., `02_ContourGeometry/LocalViewRendering.py` to `contour_local_app.py`)
#   and ensure it's in the same directory as your notebook or in Python's path.
#   Then, in a cell, execute:
#
#   from contour_local_app import ContourLocalApp
#   app = ContourLocalApp()
#   app.server.show()
#
# **Run as a Web Application (default):**
#   `python ./LocalViewRendering.py --server`
...
class ContourLocalApp(TrameApp):
    ...
    def _initialize_state(self):
        ...
        self.state.trame__title = "VTK Head Contour (Local)"
...
def main(server=None):
    ...
    app = ContourLocalApp(server=server)
    app.server.start()
    return app
